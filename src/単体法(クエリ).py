...
#単体法本体
#cB @ xB: 最適値, xB: 基底変数, cB: 基底変数の目的関数の係数: , cN: 非基底変数の目的関数の係数, B: 基底変数の制約条件の係数, N: 非基底変数の制約条件の係数, Bidx: 基底変数のindex(元の行列Aの〇列目), Nidx: 非基底変数に対応するindex(元の行列Aの〇列目)
def Simplex(cB_init, cN_init, B_init, N_init, Bidx_init, Nidx_init, b, flag):# flagは単体法ならtrue, 双対単体法
    Bidx = copy(Bidx_init)
    Nidx = copy(Nidx_init)
    cB = copy(cB_init)
    cN = copy(cN_init)
    ...
    while (1):
        ...
        if (cN_ <= 0).all() == True:
            xB = b_
            return d, xB, cB, cN, B, N, Bidx, Nidx
...
def SearchFeasibleDictionary(cB_init, cN_init, B_init, N_init, Bidx_init, Nidx_init, b):
    ...
    if bmin < 0:
        ...
        # 補助問題を解いて実行可能解を見つける
        z, _, res_cB, res_cN, res_B, res_N, res_Bidx, res_Nidx = Simplex(cB_aux, cN_aux, B_aux, N_aux, Bidx_aux, Nidx_aux, b,True)
...
# s.t. A @ x <= b
#      x >= 0
def SolveLP(cB, cN, N, B, actual_variable_idx, Nidx,Bidx,b,flag):
    # 2段階単体法
    z, xB_opt, cB, cN, B, N, Bidx, Nidx = Simplex(cB, cN, B, N, Bidx, Nidx, b, flag)

    # 得られた解から実際の変数（スラック変数以外の変数）を並び替えて出力
    x_opt = np.concatenate([xB_opt, np.zeros(cN.shape[0])])
    x_actual_opt = x_opt[np.argsort(Bidx + Nidx)][actual_variable_idx]
...
def main():
    ...
    Bidx, Nidx, cB, cN, B, N = SearchFeasibleDictionary(cB, cN, B, N, Bidx, Nidx, b)

    for t in range(T):
        # 'b' or 'c'
        tp, idx, value=input().strip().split(' ')
        # 双対単体法に移行
        if tp=='b':
            b[idx] = value
            try:
                print(SolveLP(cB, cN, N, B, actual_variable_idx, Nidx,Bidx,b,False))
            except Exception as e:
                print(e)
        # cの値を変えて再び単体法を実行
        else:
            try:
                print(SolveLP(cB, cN, N, B, actual_variable_idx, Nidx,Bidx,b,True))
            except Exception as e:
                print(e)
