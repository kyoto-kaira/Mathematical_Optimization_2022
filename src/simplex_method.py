...
def SearchFeasibleDictionary(cB_init, cN_init, B_init, N_init, Bidx_init, Nidx_init, b):
    ...
    if bmin < 0:
        ...
        k = np.argmin(b)
        swapNB(cB_aux, cN_aux, B_aux, N_aux, Bidx_aux,
               Nidx_aux, k, N_aux.shape[1] - 1)

        # 補助問題を解いて実行可能解を見つける
        z, _, res_cB, res_cN, res_B, res_N, res_Bidx, res_Nidx = Simplex(
            cB_aux, cN_aux, B_aux, N_aux, Bidx_aux, Nidx_aux, b)
        ...
        res_NB = np.concatenate([N_init, B_init], axis=1)[
            :, res_Nidx + res_Bidx]
...
def SolveLP(c, A, b):
    ...
    # 2段階単体法
    new_Bidx, new_Nidx, new_cB, new_cN, new_B, new_N = SearchFeasibleDictionary(
        cB, cN, B, N, Bidx, Nidx, b)
    z, xB_opt, cB_opt, cN_opt, B_opt, N_opt, Bidx_opt, Nidx_opt = Simplex(
        new_cB, new_cN, new_B, new_N, new_Bidx, new_Nidx, b)
...
def SolveLP2(c, A, b):
    ...
    x_opt = np.concatenate([xB_opt, np.zeros(cN_opt.shape[0])])
    x_actual_opt = x_opt[np.argsort(Bidx_opt + Nidx_opt)][actual_variable_idx]
    return z, x_actual_opt, Nidx_opt, Bidx_opt, np.linalg.inv(np.array(B_opt)) @ np.array(N_opt)
